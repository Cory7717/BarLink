        ...
        <section className="grid gap-10 lg:grid-cols-[1.1fr,0.9fr] items-center animate-fade-in">
          <div className="space-y-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row pt-4">
              ...
              <Link
                href="/explore"
                className="group px-6 py-3 rounded-lg border-2 border-white/30 bg-white/10 text-white font-semibold transition-all hover:border-white/50 hover:bg-white/20 backdrop-blur-md"
              >
                Explore bars
              </Link>
            </div>
            <div className="grid gap-3 sm:grid-cols-3 pt-4">
              {steps.map((step, idx) => (
                <div 
                  key={step.title} 
                  className={`rounded-xl border border-white/20 bg-white/10 backdrop-blur-md p-4 hover:bg-white/15 transition-all transform hover:scale-105 hover:shadow-lg hover:shadow-purple-500/20 animation-delay-${idx * 100}`}
                >
                  <h3 className="text-lg font-semibold text-white">{idx + 1}. {step.title}</h3>
        ...
        <section className="mt-20 rounded-2xl border border-white/20 bg-linear-to-br from-white/10 to-white/5 backdrop-blur-md p-8 shadow-xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, idx) => (
              <div 
                key={feature} 
                className="rounded-xl border border-white/20 bg-white/10 backdrop-blur-md p-5 text-sm text-white hover:bg-white/15 hover:border-emerald-400/50 transition-all transform hover:scale-105 hover:shadow-lg hover:shadow-purple-500/20"
              >
                <div className="flex items-start gap-3">
                  <span className="text-emerald-300 text-lg mt-1">✦</span>
          ...
          <div className="mt-8 flex flex-wrap gap-2 justify-center">
            {["Tourists", "Business travelers", "Locals"].map((audience) => (
              <span key={audience} className="rounded-full border border-white/30 bg-white/10 backdrop-blur-md px-4 py-2 text-sm text-white hover:border-emerald-400/50 hover:bg-white/15 transition-all cursor-default font-medium">
                👥 {audience}
              </span>
            ))}
          </div>
