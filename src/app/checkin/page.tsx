  ...
  return (
    <div className="min-h-screen app-shell text-white">
      ...
      <main className="mx-auto max-w-2xl px-4 pb-16 pt-24">
        <div className="glass-panel rounded-3xl p-8 shadow-lg text-center">
          {status === "processing" && (
            <>
          ...
          {status === "success" && (
            <>
              ...
              <h1 className="text-3xl font-semibold text-gradient mb-2">{message}</h1>
          ...
          {status === "error" && (
            <>
              ...
              <h1 className="text-2xl font-semibold text-red-200 mb-2">Check-in failed</h1>
              <p className="text-slate-300 mb-6">{message}</p>
              <button onClick={() => router.push("/explore")} className="btn-primary px-6 py-3 text-sm">
                Explore bars
              </button>
            </>
          )}
...
export default function CheckInPage() {
  return (
    <Suspense fallback={<div className="min-h-screen app-shell flex items-center justify-center"><div className="text-white">Loading...</div></div>}>
      <CheckInContent />
    </Suspense>
  );
}
